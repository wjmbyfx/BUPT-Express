...
### 名称:postmanCommentUser 用户评价管理员

参数名:score:评分 targetOpenid:被评分对象的openid content:评价内容 oid:对应的订单号

返回值:内部返回值



### 名称:userCommentPostman 用户评价管理员
...
### 名称:adminSendAcceptSMS 管理员通过审核发送短信
...
### 名称:adminSendRejectSMS 管理员拒绝审核发送短信
...
### 名称:postmanCommentUser 派送员给用户评分

参数名:score:评分 count:评分次数 openid 用户的openid

返回值:内部返回值



### 名称:userCommentPostman 用户给派送员评分

参数名:score:评分 count:评分次数 openid 派送员的openid
...
参数名: status(all表示全部订单) skip:分页数 从0开始 building:要查询的楼 all代表全部
